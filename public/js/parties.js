partie.storeProcedure = function (elementhandler) {
        ...
        $.ajax({
            ...
            dataType: 'json'
                ...
                .done(function (data) {
                    ...
                    partie.getData('http://elmohami.dev/parties/all/' + file_id);/* get parties by file_id*/
                ...
                if (data.length === 0)
                {
                    ...
                    for (i = 0; i < partie.length; i++) {
                        $('#parties tbody').append(
                                ...
                                '<td>' + partie[i].id + '</td>' +
                                '<td>' + 'type|nature' + '</td>' +
                                '<td>' + m.isNullAndUndef(partie[i].full_name) + '</td>' +
                                '<td>' + m.isNullAndUndef(partie[i].part_phone) + '</td>' +
                                '<td>' + m.isNullAndUndef(partie[i].created_at) + '</td>' +
                                '<td>' +
                                '<a id="part-' + partie[i].id + '" data-id="' + partie[i].id + '" class="btn btn-default btn-xs" data-toggle="modal" data-target="#modalEditPartie">تعديل<i class="fa fa-pencil"></i></a>' +
                                '<a data-id="' + partie[i].id + '" class="btn btn-warning btn-xs">حدف<i class="fa fa-remove"></i></a>' +
                                '</td>' +
                                '</tr>'
                                );
                    }
